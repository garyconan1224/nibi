...
TEMPLATES: dict[str, SummaryTemplate] = {
    ...
    "standard": SummaryTemplate(
        ...
        system_prompt=(
            ...
            "- 为每个 `## 二级标题` 在标题文字末尾追加一个时间戳，取自该章节首句所在分段的真实时间。\n"
            "- 格式必须是裸方括号 `[mm:ss]`，直接跟在标题文字后，例如：`## 外观对比 [00:42]`。\n"
            "  不要加 `*`、`-`、链接或任何前缀，也不要写成代码或粗体。\n"
            "- 只在 `##` 二级标题后加；`###` 小标题、正文、要点、表格内一律不加，保持克制。\n"
            "- 时间戳必须来自给定分段，严禁编造或估算；某标题找不到对应分段时间就不加。\n"
            "- 目的：读者点击时间戳即可跳回原片，边看视频边对照笔记。"
        ),
