...
**N6 任务级 LLM 对话上下文素材多选 chip + RAG 兜底**（P1，估时 6-8h）。

具体范围见 `docs/SPEC.md`（任务级 AI 对话）：
- 任务详情 → AI 对话 Tab 内，对话框上方提供素材 chip 多选
- 选中素材时把对应分析结果注入上下文
- 未选中或上下文超长时走 RAG 检索兜底

> 📍 **N6 开工参数**：
> - **模型**：⭐ **Opus 4.7**（涉及上下文拼接 + RAG 兜底逻辑，符合「复杂阶段」）
> - **分支**：新 worktree `feat/phase-n6-task-chat`
