...
- ✅ **文字 T**：T1 多文对比 + T2 网页抓取（含微信）+ T3 模板库已完成。
- 🔸 **图片 I**：I1 EXIF + I2 批量分析已完成；**I3 风格 DNA 决议后置**（与 R3 重叠/重算法，并入 R 阶段）。

### ⬜ P3 · 体验 + 性能优化
- ✅ R22 并行调度 / ✅ R23 性能档位 / ✅ R24·R25 VLM 提速；**F3 错误体验 ⬜ 未做**。
...
**下一步（2026-06-02 对账后 · 文字 T 已完成）**：
- **F3 错误体验优化**（当前首选，需先定切入点：限流/失败友好提示是最具体的起点）。
- 之后：[C] 复刻·AI 导演（需先补设计稿）→ [D] 开源准备。
- I3 风格 DNA 已决议后置（与 R3 重叠/重算法，并入 R 阶段）；视频路径 3 卡 `GEMINI_API_KEY` 搁置。
...
| **T** | 文字 Text | 90% | T1 多文对比 + T2 网页抓取 + T3 模板库 ✅ |
| **I** | 图片 Image | 80% | I1 EXIF + I2 批量 ✅；I3 风格 DNA 待做 |
