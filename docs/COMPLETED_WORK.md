...
## Track K · R4 — 任务中心笔记向素材直达 NoteShell + NoteShell 能力补齐（2026-06-06）

分支：`feat/k-r4-1-note-route` → `feat/k-r4-2-overview-converge` → `feat/k-r4-3-noteshell-caps`

### 问题
任务中心点素材卡片时，笔记向素材仍走旧 overview/detail 两步路由，用户体验割裂。NoteShell 缺少问AI、导出、目录 TOC 等通用笔记能力，用户必须回旧 `/ln` 或 `/av-synthesis` 才能完成常用操作。

### 影响范围
- **前端 `frontend/src/lib/resolveItemRoute.ts`**（R4.1）：笔记向（intent !== 'replica'）→ `/note`，复刻向保留原路由
- **前端 `frontend/src/pages/result/ResultsOverview/index.tsx`**（R4.2）：useEffect 兜底跳转 → NoteShell
- **前端 `frontend/src/pages/result/NoteShell/index.tsx`**（R4.3）：+238 行 — 问AI（NoteChatDrawer）+ 导出下拉（Markdown 前端下载 / Obsidian zip 后端）+ TOC 侧栏（h2/h3 提取）
- **后端 `backend/app/routes/workspaces.py`**（R4.3）：+62 行 — `GET /items/{id}/note/export?format=obsidian` item 级 Obsidian zip 导出
- **前端 `frontend/src/services/workspaces.ts`**（R4.3）：+12 行 — `exportItemNoteObsidian` service
- **测试 `tests/backend/test_item_note_write.py`**（R4.3）：+33 行 — Obsidian zip 正常导出 + 不支持 format 400

### 关键设计决策
- **resolveItemRoute 统一分流**：R4.1 起 MaterialCard / FavoritesTab / LibraryPage / ResultsOverview 共用同一 helper，路由决策集中一处
- **复刻路由不动**：replica 走原 video_detail / image_result 等，不影响现有复刻工作流
- **问AI 作用域限定**：prompt 仅含当前 note.md 正文 + 转录上下文，不泄露其他素材
- **导出 Markdown 纯前端**：Blob URL + `a.download`，零网络请求
- **导出 Obsidian 走后端**：zip 包含 frontmatter（title, source, item_id, tags），适配 Obsidian vault 结构

### 验证
- `pnpm exec tsc -b`：0 error
- `pytest`（note/workspace/exports）：53 passed
- 手测：四类型（text/image/video/audio）点卡片直达 NoteShell ✓
- 手测：Library 点击直达 NoteShell ✓
- 手测：NoteShell 三视图 / 问AI / 导出 / TOC 全部正常 ✓
- 手测：复刻向 resolveItemRoute 保留原路由 ✓
- 手测：旧链接 /ln /av-synthesis /overview 不 404 ✓
- 一键解析正常 ✓
