...
- [ ] 任务0 POC：多图单请求返回等长可解析数组（不过则停）
- [ ] 任务1 `analyze_video_frames_batch`（sf_client.py，保留单帧函数）
- [ ] 任务2 batch worker + process_video 分批 + 计数不符回退 + config 常量
- [ ] 验证：pytest（解析/回退/cancel/有序）+ 既有 72 绿
- [ ] 验证：真实视频 batch=1 vs 4 耗时 + 5 帧质量抽查（闸门）
- [ ] commit `perf(video): VLM 多帧合并单请求(多图)再提速`（不 push）
