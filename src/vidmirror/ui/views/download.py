"""Phase 2.3.a — 下载视图（从 pages/1_视频下载.py 正文搬运）。

- 已去除：st.set_page_config、st.sidebar 后端地址展示、页面级项目切换器 UI
- 保留：全部辅助函数（_is_valid_http_url / _init_state / _download_group_key 等）
- 签名：render_download_view(project_id: str) -> None
"""
...
def render_download_view(project_id: str) -> None:
    """下载视图：从 pages/1_视频下载.py 搬运而来，去除 set_page_config / sidebar / 项目切换器。"""
