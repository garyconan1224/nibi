...
# ============ 诊断 1：检查进程状态 ============
echo "【诊断 1】检查后端进程..."
if pgrep -fl "uvicorn.*8000" > /dev/null 2>&1; then
    log_success "uvicorn 进程已启动（port 8000）"
elif pgrep -fl "uvicorn" > /dev/null 2>&1; then
    log_warn "找到 uvicorn 进程，但可能未在 8000 端口"
    echo "  进程列表："
...
if [[ -f "$FE_ENV" ]]; then
    if grep -q "VITE_BACKEND_BASE_URL=http://127.0.0.1:8000" "$FE_ENV"; then
        log_success "前端 BASE URL 正确配置"
    else
        log_warn "前端 BASE URL 配置不是 8000"
...
if lsof -nP -iTCP:$BACKEND_PORT -sTCP:LISTEN 2>/dev/null | grep -q "LISTEN"; then
    ...
    echo "  快速启动命令："
    echo "    cd $PROJECT_DIR"
    echo "    python3.11 -m uvicorn backend.app.main:app --reload --port 8000"
    echo ""
    echo "  或使用启动脚本："
