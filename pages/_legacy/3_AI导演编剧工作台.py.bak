...
知识库与日志均使用当前项目目录：
- data/projects/<project_id>/json_data/
...
from shared.config import TEXT_BACKEND_ANTHROPIC, TEXT_BACKEND_OPENAI_COMPAT, get_backend_base_url, get_project_json_dir, get_project_runtime_dir
...
from shared.project_store import list_projects, load_project, make_project_id, save_project
...
st.set_page_config(page_title="AI 导演编剧工作台", page_icon="🎬", layout="wide")
_init_session()
...
default_label = next((label for label, (pid, _name) in project_map.items() if pid == project.project_id), project_labels[0])
selected_label = st.selectbox("切换项目", options=project_labels, index=project_labels.index(default_label))
switch_col, create_col = st.columns([1, 1])
with switch_col:
    if st.button("切换为所选项目", use_container_width=True):
        project_id, project_name = project_map[selected_label]
        set_current_project(project_id, project_name)
        st.rerun()
with create_col:
    new_project_name = st.text_input("新建项目名", value="", placeholder="例如：双11 手机发布会")
    if st.button("新建并切换", use_container_width=True):
        project_name = (new_project_name or "").strip() or "未命名项目"
...
st.sidebar.caption(f"后端 `{base_url}` · {'可达' if alive else '不可达'} · 模型与密钥见「系统设置」")
