...
const LNVideoPanel = forwardRef<LNVideoPanelHandle, LNVideoPanelProps>(
  ({ src, externalUrl, title, workspaceId, onTimeUpdate, onTransportChange }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      ...
      get transportNode() {
        /* 控制条 + 时间线 JSX，父组件在 player-wrap 外渲染，避免 overflow:hidden 截断 */
        return (
          <>
    ...
    useEffect(() => {
      const onFsChange = () => {
        const v = videoRef.current
        const leftPanel = v?.closest('.nibi-note-left')
        setIsFullscreen(!!leftPanel && document.fullscreenElement === leftPanel)
      }
    ...
    const toggleFullscreen = useCallback(() => {
      const v = videoRef.current
      if (!v) return
      // 全屏目标 = .nibi-note-left（含视频+控制条+转录），而非仅 .ln-video-panel
      const leftPanel = v.closest('.nibi-note-left') ?? panelRef.current
      if (!leftPanel) return
      if (document.fullscreenElement === leftPanel) {
        document.exitFullscreen()
      } else {
        leftPanel.requestFullscreen()
      }
    }, [])
